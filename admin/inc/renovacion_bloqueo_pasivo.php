...
<h3 style="text-align:center;">en los que no se renovaran creditos para pasivos</h3>
<div id="calendar"></div>
...
<script>
 ...
 // Obtener fechas ya guardadas
  $.post('inc/obtener_fechas_renovacion.php', {
    seccion: 1,
    sub_seccion: 'renovacion',
    condicion: 'pasivo'
  }, function(data) {
    const fechasGuardadas = data; // debe ser array de strings ['2025-06-01', ...]
    fechasGuardadas.forEach(fecha => {
      fechasSeleccionadas.push(fecha);
      calendar.addEvent({
        title: 'Dia Bloqueado para renocacio de creditos a pasivos',
        start: fecha,
        allDay: true,
        color: '#FF0000'
      });
