<?php
	...
	if ( $_POST['new_password'] !== $_POST['new_password_confirm'] ) {
		// Could not get the data that should have been sent.
		// die ('Please fill both the username and password field!');
		header('Location: '.BASE_URL.'login-intranet-recuperar.html?error=claves_diferentes');
	}
	...
	if ($stmt->num_rows > 0) {
		...
		if ($ok OR $_POST['password'] == $CLAVE) {
			...
			$stmt2->execute();
			$stmt2->close();
    		header('Location: '.BASE_URL.'login-intranet-recuperar.html?update_password=success');
		} else {
			header('Location: '.BASE_URL.'login-intranet-recuperar.html?error=datos_incorrectos');
		}
	} else {
