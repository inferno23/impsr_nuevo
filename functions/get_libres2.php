...
//echo $fecha.' '.$dia;
    ...
    if ($fecha<$dia) {
        $turnos.='<option disabled="disabled"> Sin Turnos dia </option>';
    }else{
        while ($hora<$fin) {
            $t++;
            $class='';
            ...
            if ($tom>0) {
                $id=str_replace(':','',$hora);
                $turnos.='<option id="hora'.$id.'" class="'.$class.'" '.$selected.' value="'.$hora.'">'.$nfecha.' / '.$hora.'</option>';
            }            
