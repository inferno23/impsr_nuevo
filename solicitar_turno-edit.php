...
global $con;
$seccion=$_SESSION['impsr']['seccion'];
//print_r($_SESSION);
$query="SELECT * FROM turnos_secciones WHERE id='$seccion'";
$secciones=$con->query($query);
echo $con->error;
if ($secciones->num_rows>0) {
    $rows=$secciones->fetch_assoc();
    $sub=$_SESSION['impsr']['subseccion'];
    $idseccion=$rows['id'];
    $subsecciones=$con->query("SELECT * FROM turnos_subsecciones WHERE id_seccion='$idseccion' AND id='$sub'");
    if ($subsecciones->num_rows>0) {
        $rowsub=$subsecciones->fetch_assoc();
        $idsub=$rowsub['id'];
        $id=$_SESSION['impsr']['idturno'];
        $datos=$con->query("SELECT * FROM turnos WHERE id='$id' ");
        if ($datos) {
            $row=$datos->fetch_assoc();
            
        }else{
            header("Location: solicitar-turnoe");
            exit();
        }
    }else{
        header("Location: solicitar-turnoe");
	...
	<div class="container pb-4" id="solicitar-turnos" >
		...
		<div class="row">
			<div class="col-12 my-4 ">
				<h2 class="h3 colort">Editar Turno</h2>
			</div>
		</div>		 
	   	<div class="row">
	   		...
	   		<div class="col-12">
    			<div class="card">
              		...
              		<div class="card-body">
                        ...
                        <form action="functions/update_turno.php" class="w-100 pb-5 pt-5" method="POST"  id="formTurno" enctype="multipart/form-data" >
                    		<input type="hidden" name="id" value="<?php echo $id; ?>">	
                    		<div class="row">
                    			<div class="col-12 col-sm-7 col-md-8">
                   					<div class="form-group">
                                    	<label for="nombre">Apellido</label>
                                    	<input type="text" class="form-control" value="<?php echo $row['apellido']?>" name="apellido" id="apellido" placeholder="Apellido (requerido)" required>
                                  	</div>
                   					<div class="form-group">
                                    	<label for="nombre">Nombre</label>
                                    	<input type="text" class="form-control" value="<?php echo $row['nombre']?>" name="nombre" id="nombre" placeholder="Nombre (requerido)" required>
                                  	</div>
                                  	<div class="form-group">
                                    	<label for="dni">DNI</label>
                                    	<input type="text" class="form-control" value="<?php echo $row['dni']?>" name="dni" id="dni" placeholder="DNI del solicitante" required>
                                  	</div>
                                  	<div class="form-group">
                                    	<label for="dni">Telefono</label>
                                    	<input type="tel" class="form-control" value="<?php echo $row['telefono']?>" name="telefono" id="telefono" placeholder="Telefono del solicitante" >
                                  	</div>
                                  	<div class="form-group">
                                    	<label for="correo">Correo</label>
                                    	<input type="email" class="form-control" value="<?php echo $row['email']?>" name="correo" id="correo" placeholder="Ingrese su correo" required>
                                  	</div>
                                  	<div class="form-group">
                                    	<label for="correo">Repetir Correo</label>
                                    	<input type="email" class="form-control" value="<?php echo $row['email']?>" name="correo" id="correo2" placeholder="Repita su correo" required>
                                  	</div>
                                  	<div class="form-row">
                                  		<div class="form-group col-9">
                                        	<label for="dni">Calle</label>
                                        	<input type="text" class="form-control" value="<?php echo $row['calle']?>" name="calle" id="calle" placeholder="Su calle" >
                                      	</div>
                                      	<div class="form-group col-3">
                                        	<label for="dni">Altura</label>
                                        	<input type="text" class="form-control" value="<?php echo $row['altura']?>" name="altura" id="altura" placeholder="Altura de la calle" >
                                      	</div>
                                  	</div>
                                  	<div class="form-row">
                                        <div class="form-group col-12">
                                            <label for="localidad">Localidad</label>
                                            <input type="text" class="form-control" value="<?php echo $row['localidad']?>" name="localidad" id="localidad">
                        				</div>

                    				</div>
                    				<?php if (!empty($row['tipo'])){ ?>
                    				<input type="hidden" name="condicion" value="<?php echo $row['tipo']; ?>">
                    				<?php if($row['tipo']=='activo') { ?>
                    				<input type="hidden" name="reciboold" value="<?php echo $row['recibo']; ?>">
                            	...
                            	<div class="col-12 col-sm-5 col-md-4">  
                                	...
                                	<div class="card mt-3">
                                		...
                                		<div class="card-body">
                                			<select name="horario" id="horarios" class="custom-select" >
                                			</select>
                                		</div>
                            		...
                            		<div class="form-group">
                                  	...
                                  	<input type="hidden" name="hora" id="hora" value="<?php echo $row['hora'];?>">
		...
		<div class="row">
			<div class="col-12">
				<div class="fondoBlanco" style="background-color: white;padding: 1.25rem;border: 1px solid rgba(0,0,0,.125);border-radius: .25rem;">
					<p>Si usted ya ha solicitado un turno, puede anularlo, modificarlo, imprimir su comprobante, o recuperar la informaciÃ³n del mismo.</p>
...
$(document).ready(function(){
	...
	$('#datepick').datepicker('setDate', '21-03-04');
	...
	$(document).on('change','#horarios',function(e){
		e.preventDefault();
		var hora=$(this).find(':selected').val();
		$('#hora').val(hora);
	});
			...
			$.ajax({
				...
				success: function(data){
					if (data.success){
						...
						$('.btn-submit').focus();
						window.location.href = "turno-solicitado";
					}
					else{
